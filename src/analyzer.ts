...
import type { RawResults, BugList, Bug } from './types.js';

const ANALYZE_PROMPT = (raw: RawResults, repoPath: string) => `
...
export async function analyzeFailures(raw: RawResults, repoPath: string): Promise<BugList> {
  if (raw.failures.length === 0) {
    return { bugs: [], totalDurationMs: raw.durationMs };
  }
  const bugs = await runClaudeJSON<Bug[]>(ANALYZE_PROMPT(raw, repoPath), { timeoutMs: 180_000 });
  return { bugs, totalDurationMs: raw.durationMs };
}
