...
import type { BugList, SiteMap, Bug, Severity } from './types.js';
...
function renderBug(b: Bug): string {
  const lines = [
    `### ${b.title}`,
    `- **URL:** ${b.page}`,
  ...
  if (b.suspectedFile) lines.push(`- **Suspected file:** \`${b.suspectedFile}\``);
  if (b.screenshotPath) lines.push(`- **Screenshot:** ${b.screenshotPath}`);
  return lines.join('\n');
}

export function renderReport(siteMap: SiteMap, list: BugList, when: Date): string {
  const counts: Record<Severity, number> = { critical: 0, high: 0, medium: 0, low: 0 };
  for (const b of list.bugs) counts[b.severity]++;
